...
with tab1:
    ...
    if properties_data:
        ...
        with col4:
            # Use city instead of neighborhood
        ...
        with col2:
            if 'price' in df.columns and not filtered_df.empty:
                ...
                if min_price == max_price:
                    # If all prices are the same, adjust range
        ...
        if not filtered_df.empty:
            ...
            # Show original data if filtered is empty
            if not df.empty:
    ...
    if properties_data and len(properties_data) > 0:
        ...
        if not df.empty:
            ...
            if has_coords and not df[['latitude', 'longitude']].isnull().all().all():
                # Create map
                try:
                    # Use median for center if available
